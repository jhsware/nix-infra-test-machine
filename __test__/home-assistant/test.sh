...
for node in $TARGET; do
  ...
  http_code=$(cmd "$node" "curl -s -o /dev/null -w '%{http_code}' http://localhost:8123/ 2>/dev/null || echo '000'")
  ...
  api_response=$(cmd "$node" "curl -s http://localhost:8123/api/ 2>/dev/null")
  ...
  manifest_response=$(cmd "$node" "curl -s http://localhost:8123/manifest.json 2>/dev/null")
  ...
  frontend_response=$(cmd "$node" "curl -s -o /dev/null -w '%{http_code}' http://localhost:8123/frontend_latest/app.js 2>/dev/null || echo '000'")
  ...
  config_exists=$(cmd "$node" "test -d /var/lib/hass && echo 'exists' || echo 'missing'")
  ...
  config_file=$(cmd "$node" "test -f /var/lib/hass/configuration.yaml && echo 'exists' || echo 'missing'")
  ...
  db_exists=$(cmd "$node" "test -f /var/lib/hass/home-assistant_v2.db && echo 'exists' || echo 'missing'")
  ...
  service_state=$(cmd "$node" "systemctl show -p SubState home-assistant --value")
  ...
  failed_units=$(cmd "$node" "systemctl list-units --failed | grep -i home || echo 'none'")
