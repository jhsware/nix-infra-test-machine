...
for node in $TEST_NODES; do
  service_status=$(cmd "$node" "systemctl is-active elasticsearch")
  if [[ "$service_status" == *"active"* ]]; then
    echo -e "  ${GREEN}✓${NC} elasticsearch: active ($node) [pass]"
  else
    echo -e "  ${RED}✗${NC} elasticsearch: $service_status ($node) [fail]"
    echo ""
    echo "Service logs:"
    cmd "$node" "journalctl -n 50 -u elasticsearch"
  fi
done
...
for node in $TEST_NODES; do
  process_status=$(cmd "$node" "pgrep -a -f elasticsearch")
  if [[ -n "$process_status" ]]; then
    echo -e "  ${GREEN}✓${NC} Elasticsearch process running ($node) [pass]"
  else
    echo -e "  ${RED}✗${NC} Elasticsearch process not running ($node) [fail]"
  fi
done
...
for node in $TEST_NODES; do
  port_check=$(cmd "$node" "ss -tlnp | grep $ELASTICSEARCH_HTTP_PORT")
  if [[ "$port_check" == *"$ELASTICSEARCH_HTTP_PORT"* ]]; then
    echo -e "  ${GREEN}✓${NC} HTTP port $ELASTICSEARCH_HTTP_PORT is listening ($node) [pass]"
  else
    echo -e "  ${RED}✗${NC} HTTP port $ELASTICSEARCH_HTTP_PORT is not listening ($node) [fail]"
  fi
done
...
for node in $TEST_NODES; do
  ...
  health_result=$(cmd "$node" "curl -s http://127.0.0.1:$ELASTICSEARCH_HTTP_PORT/_cluster/health")
  ...
  create_result=$(cmd "$node" "curl -s -X PUT 'http://127.0.0.1:$ELASTICSEARCH_HTTP_PORT/test-index' -H 'Content-Type: application/json' -d '{\"settings\": {\"number_of_shards\": 1, \"number_of_replicas\": 0}}'")
  ...
  insert_result=$(cmd "$node" "curl -s -X POST 'http://127.0.0.1:$ELASTICSEARCH_HTTP_PORT/test-index/_doc/1' -H 'Content-Type: application/json' -d '{\"name\": \"test\", \"value\": 42}'")
  ...
  query_result=$(cmd "$node" "curl -s 'http://127.0.0.1:$ELASTICSEARCH_HTTP_PORT/test-index/_doc/1'")
  ...
  echo "  Testing search..."
  search_result=$(cmd "$node" "curl -s -X GET 'http://127.0.0.1:$ELASTICSEARCH_HTTP_PORT/test-index/_search' -H 'Content-Type: application/json' -d '{\"query\": {\"match\": {\"name\": \"test\"}}}'")
  ...
  indices_result=$(cmd "$node" "curl -s 'http://127.0.0.1:$ELASTICSEARCH_HTTP_PORT/_cat/indices?v'")
